...
- بعض الأسئلة تقوم على افتراض مستحيل، مثل:
  • اجتماع رمضان والحج في وقت واحد (رمضان الشهر التاسع، وأشهر الحج شوال وذو القعدة وذو الحجة).
  • السؤال عن يوم لا وجود له (اليوم الثاني والثلاثون من رمضان، اليوم الثامن من الأسبوع).
  • السؤال عن شيء يفترض عددًا خاطئًا (الركن السادس من أركان الإسلام وهي خمسة، الشهادة الثالثة وهما شهادتان، الركعة السابعة من الفجر وهو ركعتان).
- مهم جدًا: اجعل premise_sound = false في هذه الحالات **حتى لو كانت الفتاوى المعروضة تتضمن ما يردّ الافتراض**.
  السائل هنا يحتاج تصحيح مقدمته لا جوابًا عن سؤال لا محل له.
- بيّن الخلل في premise_issue، واجعل verdict = "abstain".

ثم أصدر الحكم:

- "direct": لا تختر هذا إلا إذا كانت إحدى الفتاوى تجيب عن **هذا السؤال بعينه**،
  أي أن سؤال الفتوى هو نفس مسألة السائل لا مجرد موضوع قريب منها.
  إعادة صياغة الموضوع بالعامية، أو سؤال عن حالة أخصّ، أو عن تطبيق الحكم على وضع
  السائل — كل ذلك ليس "direct" بل "derived".
  إن ترددت بين "direct" و"derived" فاختر "derived".

- "derived": الفتاوى المعروضة تتناول أصل المسألة ويمكن بناء جواب عليها، دون أن
  تكون فتوى بعينها على حالة السائل. **هذه هي الحالة الغالبة.**

- "abstain": اختر هذا فقط في إحدى هذه الحالات:
  • الافتراض باطل (premise_sound = false)، أو
  • السؤال خارج نطاق الفتاوى الشرعية أصلًا (طبخ، برمجة، رياضة، أخبار)، أو
  • السؤال غير مفهوم أو حروف عشوائية، أو
  • لا توجد بين الفتاوى المعروضة أي فتوى تتصل بموضوع السؤال.

قواعد ملزمة:
- "abstain" إذا كان premise_sound = false.
- ضع في cited_ids أرقام الفتاوى المفيدة فعلًا فقط، ولا تضع رقمًا لا يفيد.
- **لا تختر "abstain" لمجرد عدم وجود فتوى على حالة السائل بعينها.** ما دامت هناك
  فتوى تتصل بالموضوع فاختر "derived" وقيّدها في cited_ids.
...
أمثلة محلولة (احتذِ بها حرفيًا):

مثال ١ — سؤال صحيح بصياغة عامية، والفتاوى تتناول موضوعه:
السؤال: «ودي اعرف حكم الاغاني وش رايك فيها»
الفتاوى: [1] حكم استماع الأغاني … [2] حكم المعازف …
الإخراج: {"premise_sound": true, "premise_issue": "", "verdict": "derived", "cited_ids": [1,2], "reason": "الفتاوى تتناول أصل المسألة"}
لاحظ: الافتراض سليم، فلا يجوز جعل premise_sound = false لمجرد أن الصياغة عامية أو عامة.

مثال ٢ — افتراض مستحيل:
السؤال: «اذا جا الحج ورمضان في نفس الوقت نصوم ولا نحج؟»
الإخراج: {"premise_sound": false, "premise_issue": "رمضان الشهر التاسع، وأشهر الحج شوال وذو القعدة وذو الحجة، فلا يجتمعان", "verdict": "abstain", "cited_ids": [], "reason": "افتراض مستحيل"}

مثال ٣ — خارج نطاق الفتاوى:
السؤال: «كيف أطبخ الكبسة السعودية؟»
الإخراج: {"premise_sound": true, "premise_issue": "", "verdict": "abstain", "cited_ids": [], "reason": "سؤال خارج نطاق الفتاوى الشرعية"}
لاحظ: الافتراض سليم هنا أيضًا؛ premise_sound = false تُستعمل فقط للمستحيل لا لكل امتناع.
""".strip()
